...
#         self.env_logs = os.path.join(self.log_root, '{}_{}_data'.format(self.opt.domain_name,self.opt.task_name))
...
class RobotStackFdata(Data.Dataset):
    def __init__(self,opt=None):
        self.opt = opt
        self.core_data = RobotStackdata(opt)
        self.state1A,self.state1B,self.action1 = \
            self.core_data.state1A,self.core_data.state1B,self.core_data.action1
        self.sample_num1 = self.state1A.shape[0]
