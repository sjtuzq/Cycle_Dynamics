...
class CycleGANModel():
    ...
    def set_input(self, input):
        ...
        self.gt = input[2]
        self.gt_buffer.append(self.gt.cpu().data.numpy())
    ...
    def plot_points(self,item,label):
        ...
        plt.scatter(item[:,0],item[:,1],s=3,label=label)

    def visual1(self,path):
        plt.rcParams['figure.figsize'] = (8.0, 8.0)
        plt.xlim(-4,1.5)
        plt.ylim(-4,1.5)
        realA = np.vstack(self.realA_buffer)
        fakeA = np.vstack(self.fakeA_buffer)
        recA = np.vstack(self.recA_buffer)
        realB = np.vstack(self.realB_buffer)
        fakeB = np.vstack(self.fakeB_buffer)

        self.plot_points(realA,'realA')
        self.plot_points(fakeA,'fake_B')
        self.plot_points(recA,'rec_A')
        # self.plot_points(self.real_B,'real_B')
        for p1,p2 in zip(realA,fakeA):
    ...
    def show_points(self):
        ...
        for ax_i, ax in enumerate(axes):
            if ax_i < nrows:
                ...
                ax.scatter(fakeA[:, ax_i - 2*nrows], gt[:, ax_i - 2*nrows], s=3, label='action B')
    ...
    def add_buffer(self):
        self.realA_buffer.append(self.real_A.cpu().data.numpy())
        self.fakeA_buffer.append(self.fake_B.cpu().data.numpy())
        self.recA_buffer.append(self.rec_A.cpu().data.numpy())
        self.realB_buffer.append(self.real_Bt0.cpu().data.numpy())
        self.fakeB_buffer.append(self.fake_At0.cpu().data.numpy())


    def visual2(self,path):
        # plt.rcParams['figure.figsize'] = (8.0, 8.0)
        # plt.xlim(-4,4)
        # plt.ylim(-1.5,1.5)
        self.show_points()
