...
from fmodel import Fmodel,ImgFmodel,ADmodel,AGmodel
...
class CycleGANModel():
    def __init__(self,opt):
        ...
        self.net_action_G_A = AGmodel(flag='A2B').cuda()
        self.net_action_G_B = AGmodel(flag='B2A').cuda()
        self.netF_A = Fmodel().cuda()
        self.netF_B = ImgFmodel().cuda()
        self.dataF = CDFdata.get_loader(opt)
        self.train_forward_state(pretrained=True)
        ...
        self.net_action_D_A = ADmodel().cuda()
        self.net_action_D_B = ADmodel().cuda()
        ...
        self.ImgcriterionCycle = torch.nn.MSELoss()
        self.StatecriterionCycle = torch.nn.L1Loss()
    ...
    def train_forward_img(self,pretrained=False):
        if pretrained:
            self.netF_B.load_state_dict(torch.load('./imgpred.pth'))
            return None
        ...
        for epoch in range(50):
            ...
            for i,item in enumerate(tqdm(self.dataF)):
                ...
                epoch_loss += loss.item()
            print('epoch:{} loss:{:.7f}'.format(epoch,epoch_loss/len(self.dataF)))
            torch.save(self.netF_B.state_dict(), './imgpred.pth')
        print('forward model has been trained!')
    ...
    def backward_G(self):
        ...
        loss_cycle_action_A = self.criterionCycle(rec_action_A, self.action_A) * lambda_AC * lambda_A_balance
    ...
    def show_points(self):
        ...
        # (self.fake_action_A,self.action_A)
        # (self.fake_action_B,self.action_B)
