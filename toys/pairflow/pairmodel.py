...
def Flow(input_nc=3, output_nc=3, ngf=32, n_down=6, norm='batch',
             use_dropout=False, init_type='normal', init_gain=0.02,
             gpu_id='cuda:0'):
    norm_layer = get_norm_layer(norm_type=norm)

    net = ResnetGenerator(input_nc, output_nc, ngf, n_down, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64,
        ...
        self.upnet = nn.Sequential(
            Up(1024,512,norm_layer,use_bias),
        ...
        self.outc = Outconv(ngf, 2)


    def forward(self, imgA):
        input = self.inc(imgA)
...
def define_D(input_nc=6, ndf=64, netD='basic',
...
if __name__ == '__main__':
    ...
    data = torch.rand(4,3,256,256).float().cuda()
    output = model(data)
    print(output.shape)