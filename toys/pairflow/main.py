...
from pairdata import Fdata
from pairmodel import Flow
...
def train(args):
    ...
    for epoch_id in range(args.epoch_size):
        for batch_id,item in enumerate(dataset):
            imgA = item[0].cuda().float()
            imgB = item[1].cuda().float()

            out = model(imgA)

            loss = recon_loss(out,imgB)
        ...
        torch.save(model.state_dict(),'./tmp/model_18.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_id', type=int, default=18, help='dataset id')
