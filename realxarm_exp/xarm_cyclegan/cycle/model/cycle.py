...
class CycleGANModel():
    def __init__(self,opt):
        ...
        parameters = [{'params': self.netG_A.parameters(), 'lr': self.opt.G_lr},
    ...
    def parallel_init(self,device_ids=[0]):
        ...
        self.netG_A = torch.nn.DataParallel(self.netG_A,device_ids=device_ids)
    ...
    def forward(self):
        ...
        self.real_B = Variable(self.input_Bt0).float().cuda()
    ...
    def backward_G(self):
        ...
        self.loss_state_lt0 = self.criterionCycle(self.fake_A, self.gt0)
        self.gt_buffer0.append(self.gt0.cpu().data.numpy())
        self.pred_buffer0.append(self.fake_A.cpu().data.numpy())
    ...
    def push_current_errors(self):
        ret_errors = OrderedDict([('L_t0',self.loss_state_lt0),
                                  ('D_B', self.loss_D_B), ('D_A', self.loss_D_A),
                                  ('G_B', self.loss_G_B.item()), ('G_A',  self.loss_G_A.item())])
        self.error.append(ret_errors)


    def get_current_errors(self):
        ret_errors = self.error[-1]
