...
class BoltData(Data.Dataset):
    ...
    def parse_sim_data(self):
        # sim_dir = os.path.join(self.data_root,'../gen_sim')
        # now_pos = np.load(os.path.join(sim_dir,'now.npy'))
        # act = np.load(os.path.join(sim_dir,'act.npy'))
        # nxt_pos = np.load(os.path.join(sim_dir,'nxt.npy'))
        #
        # sim_n = now_pos.shape[0]
        # assert (sim_n==act.shape[0])
        # assert (sim_n==nxt_pos.shape[0])
        # return now_pos,act,nxt_pos
        return self.now_gt.copy(),self.act_img.copy(),self.nxt_gt.copy()
